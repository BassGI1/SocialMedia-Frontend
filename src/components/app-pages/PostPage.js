...
export default function PostPage({ id }){
    ...
    useEffect(() => {
        fetch(`https://harmonise-backend-server.onrender.com/api/getpost?id=${postId}`)
        .then(res => res.json())
        .then(data => {
            ...
            fetch(`https://harmonise-backend-server.onrender.com/api/getuser?_id=${data["by"]}`)
            .then(res => res.json())
            .then(x => setPoster(x))
        })
        ...
        fetch(`https://harmonise-backend-server.onrender.com/api/getreplies?postId=${postId}&page=${0}`)
        .then(res => res.json())
        .then(data => {
            setReplies(data)
    ...
    const nextFewReplies = () => {
        ...
        if ((PlusMinus(div.scrollTop, (page*10 + 15)*replyHeight, 2) && !fetchingPage)){
            setPage(page + 1)
            setFetchingPage(true)
            fetch(`https://harmonise-backend-server.onrender.com/api/getreplies?postId=${postId}&page=${page + 1}`)
            .then(res => res.json())
            .then(data => {
                setReplies(r => [...r, ...data])
                setFetchingPage(false)
    ...
    const confirmYes = () => {
        if (!deleting){
            setDeleting(true)
            fetch("https://harmonise-backend-server.onrender.com/api/deletepost", {
                method: "POST",
                body: JSON.stringify({
                    postId: post._id
                }),
...
const CreateReply = ({ id, postId }) => {
    ...
    const reply = () => {
        if (!postText.current.length) alert("Your reply is empty!")
            ...
            setReplying(true)
            fetch("https://harmonise-backend-server.onrender.com/api/createreply", {
                method: "POST",
                headers: {"Content-Type": "application/json"},
                body: JSON.stringify({
                    postId: postId,
...
const Reply = ({userId, id, likes, text, user, created, music, postId}) => {
    ...
    const likeUnlike = () => {
        if (changeLikedStatus){
            setChangeLikedStatus(false)
            fetch("https://harmonise-backend-server.onrender.com/api/likereply", {
                method: "POST",
                body: JSON.stringify({
                    replyId: id,
                    userId: userId
    ...
    const deleteReply = () => {
        if (!deleting){
            setDeleting(true)
            fetch("https://harmonise-backend-server.onrender.com/api/deletereply", {
                method: "POST",
                body: JSON.stringify({
                    userId: userId,
                    replyId: id,
